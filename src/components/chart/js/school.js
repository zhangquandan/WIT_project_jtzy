...
export function initSchoolEcharts(myChart) {
  var option = {
    ...
    series: [{
      type: 'map3D', // 系列类型
      name: 'maprangeMap', // 系列名称
      map: 'maprange', // 地图类型。echarts-gl 中使用的地图类型同 geo 组件相同(ECharts 中提供了两种格式的地图数据，一种是可以直接 script 标签引入的 js 文件，引入后会自动注册地图名字和数据。还有一种是 JSON 文件，需要通过 AJAX 异步加载后手动注册。)
      top: '-50',
      left: '0',
      regionHeight:10,
      ...
      viewControl: getViewControl(170,30,45)
    }]
  };
